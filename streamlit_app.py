...
if authentication_status:
    ...
    if selected_main_menu == "Relative":
        ...
        if selected_sub_menu == "Relative(Trend)":
            ...
            col1, col2, col3, col4 = st.columns(4)
            with col1:
                last_n = st.number_input(label="관찰기간", value=ana_n, step=1)
        ...
        if selected_sub_menu == "유사국면분석":
            ...
            if sel_edt == df_simdt['EDATE'].max():
                if sel_simdt == fil_dt.iloc[0]:  # 첫 번째 값일 때
                    st.write("2023년 9월 19일부터 2024년 6월 10일까지 금융시장은 연방준비제도(Fed)의 금리 동결 및 추가 인상 가능성 시사, 미국의 견조한 고용 및 물가 지표, 중국의 경기 부양책, 러시아-우크라이나 전쟁 장기화 등 주요 매크로 이벤트의 영향을 받았습니다. 미국 국채 금리는 Fed의 긴축 기조와 경제 성장 기대감으로 상승했다가 금리 인상 종료 기대감으로 안정화되었으며, 신용 스프레드는 미국 투자등급 회사채의 축소와 이머징 소버린 스프레드의 변동성을 보였습니다. 달러 인덱스는 미국 경제 강세와 안전자산 선호로 강세를 유지하다 글로벌 회복 기대감에 조정을 겪었고, 미국 주식시장(S&P500)은 기술주 중심의 상승세와 기업 실적 호조로 강세를 보였습니다. 반면, 이머징 주식시장(MSCI 이머징)은 중국 경제 둔화와 신흥국 리스크로 약세를 보였으며, 금 가격은 인플레이션 우려와 안전자산 선호 심리로 상승했습니다. 국제유가는 글로벌 수요 회복 기대와 감산 정책으로 상승했으나 경기 둔화 우려로 변동성을 보이며 배럴당 70~80달러 사이에서 등락했습니다.")
                elif sel_simdt == fil_dt.iloc[1]:
                    st.write("2023년 2월 20일부터 2023년 9월 29일까지 금융시장은 연방준비제도(Fed)의 금리 인상 지속, 인플레이션 둔화 신호, 미국의 부채 한도 협상, 중국 경제 둔화, 러시아-우크라이나 전쟁 등 주요 매크로 이벤트에 영향을 받았습니다. 미국 국채 금리는 Fed의 긴축 기조와 물가 지표 개선에 따라 변동성을 보이며 10년물과 2년물 금리 모두 상승세를 보였고, 신용 스프레드는 투자등급 회사채와 이머징 소버린 채권 모두 경기 및 신용 리스크에 민감하게 반응하며 등락을 반복했습니다. 달러 인덱스는 미국 경제의 견조한 성장과 글로벌 불확실성으로 강세를 유지했으며, 미국 주식시장(S&P500)은 기술주를 중심으로 상승했지만 금리 상승 우려로 일부 조정이 있었습니다. 이머징 주식시장(MSCI 이머징)은 중국의 경기 둔화와 글로벌 위험 회피 심리로 약세를 보였고, 금 가격은 Fed 긴축과 달러 강세로 약세를 보이다 안전자산 선호 심리로 일부 반등했습니다. 국제유가는 주요 산유국의 감산 정책과 글로벌 경기 둔화 우려 사이에서 배럴당 70~90달러 수준의 변동성을 나타냈습니다.")
                elif sel_simdt == fil_dt.iloc[2]:
                    st.write("2009년 8월 3일부터 2010년 5월 7일까지는 글로벌 금융위기 이후의 경기 회복 초기 단계로, 주요 매크로 이벤트로는 미국의 경기 부양책, 중국을 비롯한 신흥국의 강한 성장세, 유럽 재정위기(그리스 위기)의 부각이 있었습니다. 미국 국채 금리는 경제 회복 기대감과 함께 상승세를 보였으나, 유럽 재정위기 여파로 안전자산 선호 심리가 강화되며 하락세로 전환되었습니다. 신용 스프레드는 경기 회복과 신용 리스크 완화로 축소되었으나, 유럽 위기로 일부 확대되기도 했습니다. 달러 인덱스는 초기에는 위험 선호 심리 확산으로 약세를 보이다가 유럽 위기의 영향으로 달러화 강세로 전환되었습니다. 미국 주식시장(S&P500)은 경기 회복에 대한 기대감으로 강세를 보였지만, 2010년 초반 유럽 재정위기로 인해 조정을 겪었습니다. 이머징 주식시장(MSCI 이머징)은 강한 성장세와 글로벌 자금 유입으로 상승했으나, 유럽 위기의 확산 우려로 변동성을 보였습니다. 금 가격은 유동성 공급과 인플레이션 우려로 강세를 지속하며 온스당 1,200달러를 넘어섰고, 국제유가는 글로벌 수요 회복 기대감으로 배럴당 70~85달러 사이에서 상승세를 이어갔습니다.")
                elif sel_simdt == fil_dt.iloc[3]:
                    st.write("2010년 8월 4일부터 2011년 2월 1일까지는 미국 연방준비제도(Fed)의 2차 양적완화(QE2) 발표, 중국의 긴축 정책, 유럽 재정위기 지속, 중동과 북아프리카 지역의 정치적 불안(아랍의 봄) 등이 주요 매크로 이벤트로 작용했습니다. 미국 국채 금리는 QE2에 따른 유동성 증가 기대감으로 하락했다가 경기 회복 신호로 상승 전환되었으며, 신용 스프레드는 경기 개선 기대감과 유동성 확대로 축소되는 모습을 보였습니다. 달러 인덱스는 QE2 발표로 약세를 보였지만, 유럽 재정위기와 중국 긴축 우려로 안전자산 선호가 강화되며 반등했습니다. 미국 주식시장(S&P500)은 양적완화로 인한 유동성 공급과 기업 실적 개선으로 강세를 나타냈으며, 이머징 주식시장(MSCI 이머징)은 초기에는 글로벌 자금 유입으로 상승했지만, 중국의 긴축 정책과 글로벌 리스크 요인으로 변동성을 보였습니다. 금 가격은 유동성 확대와 인플레이션 우려, 중동 정세 불안으로 강세를 지속하며 온스당 1,300달러를 넘어섰고, 국제유가는 글로벌 경기 회복과 중동 지역 긴장감 고조로 배럴당 80~95달러 사이에서 상승세를 이어갔습니다.")
                elif sel_simdt == fil_dt.iloc[4]:
                    st.write("2016년 5월 17일부터 2016년 11월 28일까지는 미국 대선과 트럼프 당선, 연방준비제도(Fed)의 금리 인상 기조 지속, 브렉시트(Brexit) 여파, OPEC 감산 합의 등이 주요 매크로 이벤트로 작용했습니다. 미국 국채 금리는 대선 전 불확실성으로 하락세를 보이다 트럼프 당선 후 재정지출 확대 기대감과 인플레이션 우려로 급등했습니다. 신용 스프레드는 경기 개선 기대감과 리스크 완화로 축소되었으며, 달러 인덱스는 Fed의 금리 인상 기조와 트럼프 당선 후 달러 강세 정책 기대감으로 상승했습니다. **미국 주식시장(S&P500)**은 대선 전 조정을 받았으나 당선 후 경기 부양 기대감으로 강세를 보였으며, **이머징 주식시장(MSCI 이머징)**은 대선 전 글로벌 유동성 확대와 자금 유입으로 상승했으나, 당선 후 달러 강세와 신흥국 리스크 우려로 약세 전환되었습니다. 금 가격은 대선 전 안전자산 선호로 상승했으나, 당선 후 금리 인상 기대감과 달러 강세로 하락했습니다. 국제유가는 OPEC의 감산 합의 기대감과 글로벌 수요 회복 기대에 따라 배럴당 40~50달러 수준에서 상승세를 나타냈습니다.")
                elif sel_simdt == fil_dt.iloc[5]:
                    st.write("2003년 1월 31일부터 2003년 11월 20일까지는 이라크 전쟁의 발발과 미국의 금리 인하 지속이 주요 매크로 이벤트로 작용했습니다. 미국 국채 금리는 경기 침체와 전쟁 불확실성에도 불구하고 연방준비제도(Fed)의 금리 인하 정책에 의해 낮은 수준을 유지했습니다. 신용 스프레드는 기업 실적 회복과 함께 축소되었으나, 전쟁과 글로벌 불확실성으로 여전히 높은 수준을 보였습니다. 달러 인덱스는 이라크 전쟁과 금리 인하로 약세를 보였고, **미국 주식시장(S&P500)**은 전쟁 초기에는 불확실성으로 하락했으나, 전쟁이 종료되고 경제 회복 전망이 밝아지면서 반등했습니다. **이머징 주식시장(MSCI 이머징)**은 글로벌 자금 유입과 함께 상승세를 보였으며, 금 가격은 전쟁 불확실성과 안전자산 선호 심리로 상승했으나, 전쟁 종료 후 하락세로 전환되었습니다. 국제유가는 이라크 전쟁에 대한 우려로 일시적으로 상승했으나, 전쟁의 전개와 함께 안정화되었으며, 배럴당 25~30달러 수준을 유지했습니다.")
                elif sel_simdt == fil_dt.iloc[6]:
                    st.write("2008년 11월 19일부터 2009년 3월 10일까지는 글로벌 금융위기의 심화와 미국 정부의 구제 금융 패키지 발표, 연방준비제도(Fed)의 금리 인하 및 양적완화 정책 등이 주요 매크로 이벤트로 작용했습니다. 미국 국채 금리는 금융시장의 위험 회피 심리와 Fed의 금리 인하로 크게 하락했으며, 신용 스프레드는 신용 리스크와 경제 불확실성의 확대로 급격히 확대되었습니다. 달러 인덱스는 금융위기 여파로 글로벌 위험 회피 심리가 강해지면서 강세를 보였고, **미국 주식시장(S&P500)**은 경기 침체 우려와 금융 시스템의 불안정성으로 큰 폭으로 하락했습니다. **이머징 주식시장(MSCI 이머징)**은 글로벌 경제 불안과 자본 유출로 큰 폭의 하락세를 보였으며, 금 가격은 안전자산 선호와 유동성 위기 우려로 상승했으나, 금융 시장의 전반적인 불안정성에 따라 변동성이 컸습니다. 국제유가는 글로벌 수요 감소와 금융시장 위기로 인해 배럴당 30달러 수준까지 급락했으며, 이후 공급 과잉 우려와 경기 회복 기대감에 따라 일부 반등이 있었습니다.")
            ...
            def genfig(colnm, dfa=df1, dfb=df2):
                ...
                if len(dfa) == len(dfb):
                    ...
                    dfb = dfb.iloc[indices].reset_index(drop=True)
                    dfb['dindex'] = range(1, len(dfb) + 1)
                    dfb = dfb.rename(columns={colnm: f"{colnm}_sim"})
                    dfc = pd.merge(dfa, dfb, on='dindex', how='left')
        ...
        if selected_sub_menu == "금리":
            ...
            with col1:
                ...
                st.plotly_chart(fig2, use_container_width=True)
            with col2:
                recent_sig = dfw[['DATE', 'Credit_3M', 'Act_Direc_Credit', 'Chg_Credit']].tail(10)
                html = recent_sig.to_html(index=False, border=0)
                last_row_style = '<style>table.dataframe tr:last-child { font-weight: bold; }</style>'
                html = last_row_style + html
                st.markdown(html, unsafe_allow_html=True)
